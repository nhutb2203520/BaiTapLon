      ...
      <div class="table-responsive">
        <table class="table align-middle table-hover">
          <thead class="table-light">
            <tr>
              <th>Mã</th><th>Tên</th><th>Địa chỉ</th><th>Ngày tạo</th><th class="text-center">Thao tác</th>
            </tr>
          </thead>
          <tbody>
...
const filteredNXBs = computed(() => publisherStore.listNXB?.filter(nxb => {
  const k = searchText.value.toLowerCase()
  return (nxb.TenNXB || '').toLowerCase().includes(k) || 
...
const addNXB = async () => {
  ...
  } catch { alert('Lỗi hệ thống') } finally { isAdding.value = false }
}

const cancelAdd = () => { newNXB.value = { TenNXB: '', DiaChi: '' }; showForm.value = false }
const editNXB = nxb => alert('Chức năng sửa đang phát triển')
const confirmDelete = async nxb => {
  if (confirm(`Xoá NXB "${nxb.TenNXB}"?`)) {
    const result = await publisherStore.deleteNXB(nxb.MaNXB)
    if (result.success) { alert('Xoá thành công'); await refreshData() }
